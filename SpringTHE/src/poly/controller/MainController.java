...
public class MainController {
	...
	//  양원석 수정본.  ////////////////////////////////////////////
	@RequestMapping(value="YWS/TodayNews")
	public String TodayNews() {
		
		log.info("TodayNews 시작");
		log.info("TodayNews 종료");
		
		return "/YWS/TodayNews";
	}
	@RequestMapping(value="YWS/TodaySentence")
	public String TodaySentence() {
		
		log.info("TodaySentence 시작.");
		log.info("TodaySentence 종료 ");
		
		return "/YWS/TodaySentence";
	}
	@RequestMapping(value="YWS/TodayExam")
	public String TodayExam() {
		
		log.info("TodayExam 시작!!");
		log.info("TodayExam 종료!!");
		
		return "/YWS/TodayExam";
	}
	@RequestMapping(value="YWS/TodayRecord")
	public String TodayRecord() {
		
		log.info("TodayRecord 시작!!");
		log.info("TodayRecord 종료!!");
		
		return "/YWS/TodayRecord";
	}
	@RequestMapping(value="YWS/TodayResult")
	public String TodayResult() {
		
		log.info("TodayResult 시작!!");
		log.info("TodayResult 종료!!");
		
		return "/YWS/TodayResult";
	}
	////////////////////////////////////////////////////////////
	
	@RequestMapping(value = "The/Login")
	...
	public String TheSignUpProc(HttpServletRequest request, ModelMap model) {
		...
		log.info(tDTO.getUser_id());
		log.info(tDTO.getUser_pwd());
		log.info(tDTO.getUser_email());
		log.info(tDTO.getUser_gender());
		log.info(tDTO.getUser_age());
		log.info(tDTO.getUser_interest());
		...
		// url에 /The/TheLogin을 연결하여 이동하게끔 합니다. 
		if (res > 0) {
			msg = "회원가입에 성공했습니다.";
		} else {
			msg = "회원가입에 실패했습니다.";
