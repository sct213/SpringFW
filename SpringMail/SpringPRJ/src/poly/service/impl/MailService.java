...
public class MailService implements IMailService {
	...
	final String user = "yang991211@naver.com";	//본인 네이버메일 @붙여야함!
	final String password = "Sct102412"; //본인 네이버 비밀번호 
	...
	public int doSendMail(MailDTO pDTO) {
		...
		//메일발송 성공여부(발송성공 :1 / 발송실패 : 0)
		int res = 1;
		
		//전달받은 DTO로부터 데이터 가져오기(DTO객체가 메모리에 올라가지 않아 널이 발생할 수 있기 때문에 에러방지차원으로 if문 사용 
		if(pDTO == null) {
			pDTO = new MailDTO();
		}
	...
	} catch (Exception e) { //메일 에러 다 잡기 
		res=0; //메일 발송이 실패했기 때문에 0으로 변경
		log.info("[ERROR]" + this.getClass().getName() + ".doSendMail : " + e);
		}
		
		//로그찍기 (추후 찍은 로그를 통해 이 함수 호출이 끝났는지 파악하기 용이하다.)
